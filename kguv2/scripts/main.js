...
function periodChange(elSelect, elChart, elData) {
    ...
    updateChart(elData, elChart, 3*needlen);
}
...
function shrinkHist(histData,serlen){
	...
	for ( i=0;i<histData.length;i+=20) {
		...
		let trec=(new Date('20'+dh[2],+dh[1]-1,dh[0],th[0],th[1],th[2],0)).getTime();
		if ( (trec-new Date().getTime())*120/(1000*3600) <serlen){
...
//    fetch(histurl) 
    ...
    var data = new Uint8Array(t.length / 2);
    var b;
    for (var i = 0; i < t.length; i++) {
        var b = parseInt(t[2 * i] + t[2 * i + 1], 16);
        data[i] = b;
    }
    var t = pako.inflate(data, {to: 'string'});
	        ...
	        //    console.log("\n+++++mac30 data  OK    :\n" + t);
	            ...
	            let l1=histData;
            chartD = histData;
            shrinkHist(chartD,serlen)
            let l2=chartD.length;
            console.log('serlen='+serlen+' l1='+l2+' l2='+l2);
            chart.validateData();
